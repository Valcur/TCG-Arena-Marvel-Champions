...
async function getVillainDeckTopCard() {
  ...
  if (!deck || deck?.length === 0) {
    ...
    for (const card of newEncounterDeck) {
      ...
      card.position.section = "EncounterDeck"
    }
...
If a card instructs a player to put a nemesis side scheme
into play, the nemesis side scheme associated with the
player that drew the card is placed in the play area, and
their nemesis minion is put into play engaged with that
player. The other encounter cards associated with that
nemesis are placed in the encounter discard pile.
Each hero’s nemesis, side scheme, and associated cards
